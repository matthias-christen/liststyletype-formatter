require('should');
var Formatter = require('../lib/main');

var type = 'lower-greek';

function format(v)
{
    return Formatter.format(v, type);
}


describe(type, function()
{
    it('should format 1', function()
    {
        format(1).should.eql('α.');
    });

    it('should format 1.1', function()
    {
        format(1.1).should.eql('α.');
    });

    it('should format 5', function()
    {
        format(5).should.eql('ε.');
    });

    it('should format 11', function()
    {
        format(11).should.eql('λ.');
    });

    it('should format 24', function()
    {
        format(24).should.eql('ω.');
    });

    it('should format 25', function()
    {
        format(25).should.eql('αα.');
    });

    it('should format 49', function()
    {
        format(49).should.eql('βα.');
    });    

    it('should format 255', function()
    {
        format(255).should.eql('κο.');
    });

    it('should format 0', function()
    {
        format(0).should.eql('0.');
    });

    it('should format -5', function()
    {
        format(-5).should.eql('-5.');
    });
});
